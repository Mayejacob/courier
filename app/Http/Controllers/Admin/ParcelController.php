...
class ParcelController extends Controller
{
    ...
    {
        $parcels = Parcels::latest()->paginate(10);
        return view('admin.parcel.index',compact('parcels'));
    }
    public function add()
    {
        $branches = Branch::latest()->get();
        return view('admin.parcel.add',compact('branches'));
    }
    ...
    public function view($id)
    {
        ...
        return view('admin.parcel.view',compact('parcel'));
    }
    public function edit($id)
    {
        $parcel = Parcels::find($id);
        $branches = Branch::latest()->get();
        return view('admin.parcel.edit',compact('parcel','branches'));
    }
    public function track()
    {
        return view('admin.parcel.track');
    }
    public function show(Request $request)
    {
        $searchTerm = '%'.$request->ref_no.'%';
            
        return view('admin.parcel.track',['tracks' => Tracks::where('reference_number', 'like', $searchTerm)->get()
        ]);
    }
    ...
    {
        $startDate = Carbon::parse($request->fromDate)->toDateTimeString();
        $endDate = Carbon::parse($request->toDate)->toDateTimeString();
        $parcels = Parcels::whereBetween('created_at', [$startDate, $endDate])->paginate(10);
        return view('admin.reports',compact('parcels'));
    }
}
